...
# Load data
brain_stats <- read_csv("Desktop/lme_20230615_test/BLSA_Brain_stats_concat_20221110_delivery.csv")
motion_stats <- read_csv("Desktop/lme_20230615_test/BLSA_eddy_movement_rms_average_20221109_w_scanner_id.csv")
...
for (s_atlas in list_atlas) {
  ...
  path_LUT = sprintf("Desktop/lme_20230615_test/LUT_customize/%s_LUT.csv", s_atlas)
  ...
  for (idx_region in 1:nrow(LUT)) {
    for (diff_type in list_diff_type) {
      for (value_type in list_value_type) {
        ...
        fn_save = sprintf("Desktop/lme_20230615_test/part/%s-%s-%s.csv", s_atlas, s_region_id, s_measure)
        ...
        fn_save <- sprintf("Desktop/lme_20230615_test/stats/coef_ind/%s-%s-%s_coef.csv", s_atlas, s_region_id, s_measure)
        write.csv(coef_df, fn_save, row.names=TRUE, quote=FALSE)
        
        coef_sum_df <- as.data.frame(coef(summary(m_final)))
        fn_save <- sprintf("Desktop/lme_20230615_test/stats/coef_sum/%s-%s-%s_coef_summary.csv", s_atlas, s_region_id, s_measure)
        write.csv(coef_sum_df, fn_save, row.names=TRUE, quote=FALSE)
        
        ranef_df <- as.data.frame(ranef(m_final)$Subject_ID)
        fn_save <- sprintf("Desktop/lme_20230615_test/stats/ranef_ind/%s-%s-%s_ranef.csv", s_atlas, s_region_id, s_measure)
        ...
        fn_save <- sprintf("Desktop/lme_20230615_test/stats/ranef_sum/%s-%s-%s_ranef_summary.csv", s_atlas, s_region_id, s_measure)
