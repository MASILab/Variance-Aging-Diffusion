  ...
  for (idx_region in 1:nrow(LUT)) {
    for (diff_type in list_diff_type) {
      for (value_type in list_value_type) {
        ...
        # # hard code the formula for debugging
        # if (s_measure=='RD_std') {
        #   m <- lmer(RD_std ~ Age_base + Interval + Motion + Sex + DTI_ID + (1 | Subject_ID), data=df, REML=TRUE)
        # } else if (s_measure=='AD_std') {
        #   m <- lmer(AD_std ~ Age_base + Interval + Motion + Sex + DTI_ID + (1 | Subject_ID), data=df, REML=TRUE)
        # } else if (s_measure=='FA_std') {
        #   m <- lmer(FA_std ~ Age_base + Interval + Motion + Sex + DTI_ID + (1 | Subject_ID), data=df, REML=TRUE)
        # } else if (s_measure=='MD_std') {
        ...
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data/stats/coef_ind/%s-%s-%s_coef.csv", s_atlas, s_region_id, s_measure)
        write.csv(coef_df, fn_save, row.names=TRUE, quote=FALSE)
        
        coef_sum_df <- as.data.frame(coef(summary(m_final)))
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data/stats/coef_sum/%s-%s-%s_coef_summary.csv", s_atlas, s_region_id, s_measure)
        write.csv(coef_sum_df, fn_save, row.names=TRUE, quote=FALSE)
        
        ranef_df <- as.data.frame(ranef(m_final)$Subject_ID)
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data/stats/ranef_ind/%s-%s-%s_ranef.csv", s_atlas, s_region_id, s_measure)
        ...
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data/stats/ranef_sum/%s-%s-%s_ranef_summary.csv", s_atlas, s_region_id, s_measure)
