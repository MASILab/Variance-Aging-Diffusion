...
# In the formula for lme, Age and Interval are used together.
  ...
  for (idx_region in 1:nrow(LUT)) {
    for (diff_type in list_diff_type) {
      for (value_type in list_value_type) {
        ...
        fn_save = sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data_20230201/part/%s-%s-%s.csv", s_atlas, s_region_id, s_measure)
        ...
        # Backward regression
        formula_full_model <- sprintf("%s ~ Age + Interval + Motion + Sex + DTI_ID + (1 | Subject_ID)", s_measure)
        ...
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data_20230201/stats/coef_ind/%s-%s-%s_coef.csv", s_atlas, s_region_id, s_measure)
        write.csv(coef_df, fn_save, row.names=TRUE, quote=FALSE)
        
        coef_sum_df <- as.data.frame(coef(summary(m_final)))
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data_20230201/stats/coef_sum/%s-%s-%s_coef_summary.csv", s_atlas, s_region_id, s_measure)
        write.csv(coef_sum_df, fn_save, row.names=TRUE, quote=FALSE)
        
        ranef_df <- as.data.frame(ranef(m_final)$Subject_ID)
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data_20230201/stats/ranef_ind/%s-%s-%s_ranef.csv", s_atlas, s_region_id, s_measure)
        ...
        fn_save <- sprintf("/home/local/VANDERBILT/gaoc11/Projects/Variance-Aging-Diffusion/Data_20230201/stats/ranef_sum/%s-%s-%s_ranef_summary.csv", s_atlas, s_region_id, s_measure)
